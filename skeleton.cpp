                         ...
                         Intersection &closestIntersection)
{
  ...
  for (int i = 0; i < triangles.size(); i++)
  {
    ...
    // equations 7,8,9 & 11, they all have to be true for intersection to occur
    if (u >= 0 && v >= 0 && u + v <= 1 && t >= 0)
    {
      ...
      if (distance < m)
      {
        // m is updated with the distance. if not done, objects will not be placed in right order.
        m = distance;
        // closest intersection is updated with 3d position and other info about intersection
...
void Update()
{
  ...
  // controls camera. i am not sure of the signs for change, i think this is an arbitrary choice.
...
vec3 DirectLight(const Intersection &i)
{
  ...
  // vectors. Since we do not allow negative light we take the max of the projected value and zero.
  Intersection intersection;
